...
class RtspServer(
    context: Context, private val connectCheckerRtsp: ConnectCheckerRtsp,
    val port: Int
) {
    ...
    fun startServer() {
        stopServer()
        thread = Thread {
            server = ServerSocket(port, -1, InetAddress.getByName(serverIp))
    ...
    internal class Client(
        ...
        isStereo: Boolean, user: String?, password: String?
    ) : Thread() {
